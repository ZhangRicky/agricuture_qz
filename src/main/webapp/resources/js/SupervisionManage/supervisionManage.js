		...
		addUserClick : function() {
			$("#addUser").on(
							...
							function() {
								...
								$("#userId,#realName,#constant,#userName,#password,#confirmpassword,#mobile,#email,#description").val("");
								//获取所属机构
	...
	var bindingDepartmentWindow = {
		...
		initTreeList: function(data){
			...
			$.ajax({
				cache:false,
				type : "get",
				url : "user/departmentTree/"+data.id,
				dataType : "json",
				contentType : "application/json;charset=UTF-8",
		...
		saveDepartmentClick : function() {
			$("#saveDepartment").on("click", function(e) {
				...
				$.ajax({
					cache:false,
					type : "post",
					url : "user/"+ params.userId+"/departments/" + params.deptIds,
					dataType : "json",
					contentType : "application/json;charset=UTF-8",
					success : function(data, textStatus, jqXHR) {
...
//						height: 540,
    ...
    var onCheck = function(checkedNodes) {
        var checkedNodes = checkedNodes,
            treeView = $("#departmentList").data("kendoTreeView");
